...
function hourSeq(hour) { return (hour === 23 || hour === 0) ? 1 : Math.floor((hour + 1) / 2) + 1; }
function yearBranchSeq(lunarYear) { return ((lunarYear - 4) % 12 + 12) % 12 + 1; }
function timeNumbers(input) { const lunar = solarToLunar(input.year, input.month, input.day); return { yearNum: yearBranchSeq(lunar.year), monthNum: lunar.month, dayNum: lunar.day, hourNum: hourSeq(input.hour) }; }
...
export function zaobiQiGua(input, seed = 0, birth, basis) {
    ...
    const total = base + seed, lines = [], moving = [];
    for (let i = 0; i < 6; i++) {
...
function bitsToTrigram(a) { const bits = (a[0] ? 4 : 0) + (a[1] ? 2 : 0) + (a[2] ? 1 : 0); return [7, 3, 5, 1, 6, 2, 4, 0][bits] ?? 0; }
export function trigramName(idx) { return TRIGRAMS[idx] || ''; }
