...
import { fullPaipan } from '../engine.js';
import { buildBaziPanel } from '../panels/bazi.js';
const $ = (id) => document.getElementById(id);
const history = [];
let snapshotId = 0;
let activeTab = 'overview';
const BASIS_ZH = { time: '仅时间', time_bazi: '时间+八字', bazi: '仅八字' };
const BASIS_SUFFIX = { time: '', time_bazi: '+八', bazi: '八' };
const METHOD_HINTS = {
...
async function askAI(state, syn, q, key) {
    ...
    if (!res.ok)
        throw new Error('AI请求失败：' + res.status);
    const data = await res.json();
    return data.choices?.[0]?.message?.content || '（AI未返回内容）';
}
function addSnapshot(input, method, basis, state, syn) {
...
function renderCompareTabs() {
    ...
    el.querySelectorAll('.compare-tab').forEach(t => t.addEventListener('click', () => { activeTab = t.dataset.tab || 'overview'; renderCompareTabs(); renderCompareTable(); }));
}
function renderCompareTable() {
    ...
    if (!el)
        return;
    if (history.length === 0) {
        el.innerHTML = '<tr><td class="error">暂无记录</td></tr>';
        return;
    }
    ...
    if (activeTab === 'overview')
        ...
        rows = [['结果', history.map(h => h.state.panels.xiaoliu.result)], ['五行', history.map(h => h.state.panels.xiaoliu.element)], ['路径', history.map(h => h.state.panels.xiaoliu.path.join('->'))]];
    else if (activeTab === 'synthesis')
        rows = [['趋势', history.map(h => h.synthesis.trend)], ['评分', history.map(h => String(h.synthesis.score))], ['概要', history.map(h => h.synthesis.summary.substring(0, 30) + '...')], ['要点', history.map(h => h.synthesis.keyPoints.join('；').substring(0, 30) + '...')]];
    const body = rows.map(([k, v]) => `<tr><td class="col-key">${k}</td>${v.map(x => `<td>${x}</td>`).join('')}</tr>`).join('');
    el.innerHTML = `<tr><th>项目</th>${hdr}</tr>${body}`;
}
function showCompare() { const s = $('compare-section'); if (s) {
...
function bindEvents() {
    ...
    $('run-btn')?.addEventListener('click', () => {
        const out = $('output');
        if (out)
            out.innerHTML = '<div class="loading">推演中</div>';
        setTimeout(() => {
            try {
                const { input, method, nums, basis, birth, gender } = readForm();
                if (method === 'cuanke' && nums.length !== 6)
                    throw new Error('铜钱摇卦需输入6次结果(0-3)');
                const { state, synthesis } = fullPaipan(input, method, nums, 0, birth, basis, gender);
                addSnapshot(input, method, basis, state, synthesis);
                if (out)
                    out.innerHTML = renderPanels(state, synthesis);
                showCompare();
                out?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                const key = $('apikey')?.value || '';
                const q = $('question')?.value || '';
                if (key && q) {
                    askAI(state, synthesis, q, key).then(c => { if (out)
                        out.innerHTML += renderAIResult(c); }).catch(e => { if (out)
                        out.innerHTML += `<section class="panel ai"><h2>AI 解卦</h2><p>失败：${e.message}</p></section>`; });
                }
            }
            catch (e) {
                if (out)
...
function init() { setDefaultTime(); updateBasisVisibility(); updateMethodHint(); bindEvents(); }
