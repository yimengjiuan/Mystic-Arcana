// 农历数据表 1900-2050
const LUNAR_DATA: readonly string[] = [
...
function getLunarData(year: number): number {
  ...
  return parseInt(LUNAR_DATA[idx], 16) || 0;
}
...
export function solarToLunar(y: number, m: number, d: number): { year: number; month: number; day: number; isLeap: boolean } {
  ...
  let offset = Math.floor((target - LUNAR_BASE) / 86400000);
  ...
  let isLeap = false;
  while (lunarMonth <= 12) {
    let days = isLeap ? leapDays(lunarYear) : monthDays(lunarYear, lunarMonth);
