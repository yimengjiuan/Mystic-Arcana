...
export function getNajia(upperName: string, lowerName: string): string { const u = INNER_OUTER_TRIGRAMS[TRIGRAM_INDEX[upperName]], l = INNER_OUTER_TRIGRAMS[TRIGRAM_INDEX[lowerName]]; if (!u || !l) throw new Error('invalid trigram'); return l.inner + u.outer; }
export function parseNajia(najia: string) { if (najia.length !== 12) throw new Error('najia must be 12 chars'); return Array.from({ length: 6 }, (_, i) => ({ gan: najia[i * 2], zhi: najia[i * 2 + 1], ganzhi: najia.slice(i * 2, i * 2 + 2) })); }
...
export function hourToDizhi(hour: number): string { if (hour === 23 || hour === 0) return '子'; for (const t of DIZHI_TIME) if (hour >= t.start && hour < t.end) return t.name; return '子'; }
export const CANGGAN_STRING = '癸,己癸辛,甲丙戊,乙,戊乙癸,丙戊庚,丁己,己丁乙,庚壬戊,辛,戊辛丁,壬甲';
export function parseCanggan(zhi: string): readonly string[] { const idx = DIZHI_LIST.indexOf(zhi); if (idx < 0) return []; return CANGGAN_STRING.split(',')[idx]?.split('').filter(c => c) || []; }
