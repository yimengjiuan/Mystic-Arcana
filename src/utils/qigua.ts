...
function timeNumbers(input: TimeInput) {
  ...
  return { yearNum: yearBranchSeq(lunar.year), monthNum: lunar.month, dayNum: lunar.day, hourNum: hourSeq(input.hour) };
}
...
export function cuankeQiGua(throws: readonly number[]) {
  ...
  const lines: boolean[] = [], moving: number[] = [];
  for (let i = 0; i < 6; i++) {
    if (throws[i] === 0) { lines.push(false); moving.push(i + 1); }
    else if (throws[i] === 3) { lines.push(true); moving.push(i + 1); }
    else lines.push(throws[i] === 1);
  }
  return { lines, moving };
}
