...
export function liuQinOf(zhiSelf: string, zhiOther: string, dayGan: string): string {
  ...
  if (selfEl === otherEl) {
    if (WUXING[dayGan] === selfEl) return '比肩';
    return '比和';
  }
...
export function buildLines(
  ...
  const upperName = TRIGRAMS[h.upper] || '';
  const lowerName = TRIGRAMS[h.lower] || '';
  const najiaStr = getNajia(upperName, lowerName);
...
export function buildHexagram(
  ...
  const h = getHexagramByIndex(hexIndex);
  const lines = buildLines(hexIndex, moving, dayGan);
  return {
    index: hexIndex,
    name: h.name,
    fullName: h.fullName,
