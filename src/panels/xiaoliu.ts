...
export function buildXiaoLiu(bazi: Bazi, _hour: number): XiaoLiuPanel {
  let pos = 0, path = ['大安'];
  pos = (pos + bazi.lunar.month - 1) % 6; path.push(PALACE_DATA[pos].name);
  pos = (pos + bazi.lunar.day - 1) % 6; path.push(PALACE_DATA[pos].name);
  const hourIdx = DIZHI_ORDER.indexOf(bazi.hour.zhi);
  pos = (pos + (hourIdx >= 0 ? hourIdx + 1 : 1) - 1) % 6; path.push(PALACE_DATA[pos].name);
  const final = PALACE_DATA[pos];
  return { monthGanZhi: bazi.month.ganzhi, dayGanZhi: bazi.day.ganzhi, hourGanZhi: bazi.hour.ganzhi, path, result: final.name as XiaoLiuPanel['result'], element: final.element, meaning: final.meaning };
}
