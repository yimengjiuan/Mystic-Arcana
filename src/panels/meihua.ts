 ...
 * ------------------------------------------------------------------
 * 梅花易数核心：体用之分。
 *   不动之卦为"体"（自身），动之卦为"用"（所测之事）。
 *   以体用五行生克定吉凶：用生体大吉，体克用次吉，体生用小凶，用克体凶。
 */
import type { Hexagram, MeiHuaPanel } from '../types';
import { findChangedHexagram, findHuHexagram } from '../utils/parser';
import { getHexagramByIndex, findHexagramByTrigrams, TRIGRAM_ELEMENT } from '../data/hexagrams';
...
export function buildMeiHua(ben: Hexagram, dong: readonly number[]): MeiHuaPanel {
  ...
  if (idx === 1) interp = '体生用，小凶，付出多收获少。';
  else if (idx === 2) interp = '体克用，所谋易成但费力。';
  else if (idx === 3) interp = '用克体，所谋难成，防损耗。';
  else if (idx === 4) interp = '用生体，大吉，百事可成。';
