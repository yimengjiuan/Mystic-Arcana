...
export function buildLiuYao(ben: Hexagram, dong: readonly number[], bazi: Bazi): LiuYaoPanel {
  ...
  const changed = raw.lines.map((b, i) => dong.includes(i + 1) ? !b : b);
  const bitsToIdx = (a: readonly boolean[]) => (a[0] ? 4 : 0) + (a[1] ? 2 : 0) + (a[2] ? 1 : 0);
  ...
  const liuQinMap = ben.lines.map((l, i) => {
    ...
    if (el === dayEl) lq = '比肩';
      ...
      if (sheng[dayEl] === el) lq = '食神';
      else if (ke[dayEl] === el) lq = '财爻';
      else if (sheng[el] === dayEl) lq = '印爻';
      else if (ke[el] === dayEl) lq = '官鬼';
    }
